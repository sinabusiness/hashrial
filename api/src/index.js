...
function isValidBtcAddress(addr) {
  ...
  // FIX: the previous BigInt-based decode silently drops leading zero bytes.
  // Every P2PKH/P2SH address has at least one leading zero byte (the version
  // byte, which is why they start with '1'), but any address whose 20-byte
  // hash payload ALSO happens to start with 0x00 has a SECOND leading zero
  // byte, shown as a second leading '1' character. BigInt math has no way to
  // represent that — leading zeros contribute nothing to the numeric value —
  // so the decoded byte length comes out short, and slicing off the last 4
  // bytes as "the checksum" no longer lines up correctly. This decodes to an
  // explicit byte array and re-prepends exactly as many zero bytes as there
  // are leading '1' characters, which is the standard, correct approach.
  ...
  try {
    ...
    const hash1 = crypto2.createHash("sha256").update(payload).digest();
    const hash2 = crypto2.createHash("sha256").update(hash1).digest();

    return Buffer.compare(checksum, hash2.subarray(0, 4)) === 0;
  } catch { return false; }
}
...
// ── Auth middleware ───────────────────────────────────────────
// FIX: token revocation was previously a separate `checkRevoked` middleware
// that was only ever wired onto ONE route (/api/auth/change-password). Every
// other authenticated endpoint skipped it entirely, so logging out or
// resetting a password never actually stopped a stolen token from working
// anywhere else. Folding the check directly into `auth` means every route
// that already uses `auth` gets this protection automatically — no risk of
// forgetting to chain a second middleware on a new route later.
//
// This also checks password_changed_at against the token's `iat` (issued-at)
// claim, which jsonwebtoken adds automatically. This is what makes password
// reset/change actually invalidate ALL previously-issued tokens for that
// user, not just a single blacklisted jti — no need to track every issued
// token individually.
async function auth(req, res, next) {
  ...
  try {
    // Explicit single-session revocation (from /api/auth/logout)
    if (decoded.jti) {
      const bl = await pg.query(
        "SELECT 1 FROM token_blacklist WHERE jti=$1 AND expires_at > NOW()",
        [decoded.jti]
      );
      if (bl.rows.length > 0) return res.status(401).json({ error: "Token revoked" });
    }

    // Blanket revocation of every token issued before the last password change
    const u = await pg.query("SELECT password_changed_at FROM users WHERE id=$1", [decoded.id]);
    const changedAt = u.rows[0]?.password_changed_at;
    if (changedAt && decoded.iat && decoded.iat * 1000 < new Date(changedAt).getTime()) {
    ...
    // Fail open on a transient DB error rather than locking out every user —
    // the JWT signature and expiry checks above still apply regardless.
    console.error("auth revocation check error:", e.message);
    req.user = decoded;
    next();
  }
}
...
app.post("/api/auth/reset-password", async (req, res) => {
  ...
  try {
    ...
    // NOTE: no explicit token revocation needed here. password_changed_at was
    // just set above, and the auth middleware now rejects any token whose
    // `iat` predates it — every previously-issued token is invalidated
    // automatically the next time it's used, across every device/session.
    res.json({ ok: true });
  } catch (e) {
...
app.post("/api/auth/change-password", auth, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // NOTE: the previous DELETE FROM token_blacklist here was backwards — it
    // erased revoked entries instead of adding one, and deleting them doesn't
    // invalidate anything else anyway. password_changed_at=NOW() above is
    // what actually revokes every existing session now — see the auth
    // middleware's iat comparison.
