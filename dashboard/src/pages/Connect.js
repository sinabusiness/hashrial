...
export default function Connect() {
  ...
  if (!info) return <div style={{ padding:40, textAlign:"center", color:"var(--text2)" }}>Loading…</div>;

  const username = `${info.username?.split(".")?.[0]}.${workerName||"WORKER_NAME"}`;

  return (
    <div style={{ padding:"24px 28px", maxWidth:780 }}>
      <h1 style={{ fontSize:22, fontWeight:700, marginBottom:4 }}>Connect Your Miners</h1>
      <div style={{ color:"var(--text2)", fontSize:13, marginBottom:22 }}>Configure your ASIC or mining software to connect to Hashrial</div>
      ...
      <div style={{ background:"rgba(247,147,26,0.06)", border:"1px solid rgba(247,147,26,0.18)", borderRadius:"var(--r2)", padding:"13px 16px", marginBottom:20, fontSize:13, color:"var(--text2)", lineHeight:1.7 }}>
        <strong style={{ color:"var(--accent)" }}>2% Pool Fee</strong> — Hashrial charges 2% for infrastructure. Your remaining 98% earnings are credited to your account and visible in the Earnings page.
      </div>

      <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:"var(--r2)", padding:"20px 22px", marginBottom:16 }}>
        <h2 style={{ fontSize:14, fontWeight:600, marginBottom:14 }}>Step 1 — Name your worker</h2>
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <input value={workerName} onChange={e => setWorkerName(e.target.value.replace(/[^a-zA-Z0-9_-]/g,""))}
            placeholder="e.g. rig01, asic1"
      ...
      <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:"var(--r2)", padding:"20px 22px", marginBottom:16 }}>
        ...
        <CopyBox label="Stratum URL" value={info.stratum} />
        <CopyBox label="Username / Worker" value={username} />
