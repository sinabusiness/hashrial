...
export default function WorkerDetail() {
  ...
  if (!data?.worker) return (
    <div style={{ padding:40 }}>
      <Link to="/workers" style={{ color:"var(--accent)", fontSize:13 }}>← Back to Workers</Link>
      <div style={{ marginTop:16, color:"var(--text2)" }}>Worker not found.</div>
    </div>
  );
  ...
  return (
    <div style={{ padding:"24px 28px", maxWidth:1100 }}>
      <Link to="/workers" style={{ color:"var(--text2)", fontSize:13, display:"inline-flex", alignItems:"center", gap:4, marginBottom:16 }}>← Back to Workers</Link>
      ...
      <div style={{ color:"var(--text2)", fontSize:12.5, marginBottom:22 }}>
        ...
        Last seen: {worker.last_seen  ? new Date(worker.last_seen).toLocaleString()       : "—"}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12, marginBottom:20 }}>
        {[
          ...
          { label:"Accepted",     value:parseInt(latest.accepted||0).toLocaleString(), color:"var(--green)" },
          { label:"Stale",        value:parseInt(latest.stale||0).toLocaleString(),    color:parseInt(latest.stale||0)>0?"var(--yellow)":"var(--text2)" },
        ].map(s => (
      ...
      <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:"var(--r2)", padding:"18px 22px", marginBottom:16 }}>
        <h2 style={{ fontSize:14, fontWeight:600, marginBottom:14 }}>Hashrate (24h)</h2>
        {chartData.length === 0
          ? <div style={{ height:180, display:"flex", alignItems:"center", justifyContent:"center", color:"var(--text2)", fontSize:13 }}>No data yet for this worker.</div>
      ...
      {chartData.length > 0 && (
        <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:"var(--r2)", padding:"18px 22px" }}>
          <h2 style={{ fontSize:14, fontWeight:600, marginBottom:14 }}>Shares (24h)</h2>
          <ResponsiveContainer width="100%" height={140}>
            <BarChart data={chartData} margin={{ top:4, right:4, left:-10, bottom:0 }}>
              ...
              <Bar dataKey="accepted" fill="#2ea84c" radius={[2,2,0,0]} name="Accepted" />
              <Bar dataKey="stale"    fill="#d4a017" radius={[2,2,0,0]} name="Stale" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
