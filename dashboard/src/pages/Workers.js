...
export default function Workers() {
  ...
  const load = useCallback(() => {
    return api.workers()
      .then(ws => { setWorkers(ws || []); setLastRefresh(new Date()); })
      .catch(console.error);
  }, []);
    ...
    <div style={{ padding:"24px 28px", maxWidth:1400 }}>
      <div style={{ marginBottom:20, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <div>
          ...
          <div style={{ color:"var(--text2)", fontSize:13 }}>All miners connected to your account</div>
        </div>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:18 }}>
        {[
          { label:"Total",   count:workers.length, color:"var(--text)" },
          { label:"Online",  count:online,          color:"var(--green)" },
          { label:"Offline", count:offline,          color:"var(--red)" },
        ].map(s => (
      ...
      <div style={{ background:"var(--bg2)", border:"1px solid var(--border)", borderRadius:"var(--r2)", overflow:"hidden" }}>
        {loading ? (
          <div style={{ padding:40, textAlign:"center", color:"var(--text2)" }}>Loading workers…</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding:40, textAlign:"center", color:"var(--text2)" }}>
            {filter === "all"
              ? <><div style={{ fontSize:28, marginBottom:8 }}>⛏</div><div>No workers yet. <Link to="/connect">Connect your first miner →</Link></div></>
              : `No ${filter} workers`}
          </div>
        ) : (
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ borderBottom:"1px solid var(--border)", background:"var(--bg3)" }}>
                {["Worker","Status","10m","1h","24h","Accepted","Stale","Last Seen"].map(h => (
                  <th key={h} style={{ textAlign:"left", padding:"9px 14px", fontSize:10, color:"var(--text2)", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.4px" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(w => (
                <tr key={w.worker_name} style={{ borderBottom:"1px solid rgba(255,255,255,0.03)" }}
                  ...
                  <td style={{ padding:"10px 14px" }}>
                    <Link to={`/workers/${w.worker_name}`} style={{ color:"var(--text)", fontFamily:"var(--mono)", fontSize:12, fontWeight:500 }}>{w.worker_name}</Link>
                  </td>
